...
export class DataTableComponent implements OnInit, AfterViewInit, OnChanges {
  ...
  ngOnInit(): void {
    ...
    this.displayedColumns = this.displayedColumnsConfig.map(
      (value: any, index) => value.columnLabel
    );
    if (this.config.isSelectable) {
      this.selection = new SelectionModel<any>(this.config.isMultipleSelect, [
        ...this.dataSource?.data.filter(
          (row) =>
            this.config.preSelectedItems.indexOf(
              row[this.config.selectionColumn]
            ) > -1
        ),
      ]);

...
export class AuthenticationService {
  ...
  authentication(
    params: RequestModel
  ): Observable<ApiResponseModel<UserAuthenticationModel>> {
    const bodyParams = params.authModel;
    const bodyParamsWithoutUndefined: any = {};
    Object.entries(bodyParams || {}).forEach(([key, value]: [string, any]) => {
      if (value !== undefined) bodyParamsWithoutUndefined[key] = value;
    });
    return this.http.post<ApiResponseModel<UserAuthenticationModel>>(
  ...
  validateToken(): Observable<ApiResponseModel<UserAuthenticationModel>> {
    return this.http.get<ApiResponseModel<UserAuthenticationModel>>(

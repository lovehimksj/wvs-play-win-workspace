...
import { MasterState, SetPlayerList } from '@wvs-play-win-workspace/data-access-store';
...
export class PlayerFacadeImplService implements PlayerFacadeService {
  ...
  @Select (MasterState.getPlayerList) load_player_list$: Observable<PlayerServerModel[]>;
  setAllPlayer() {
    this._playerService.getAll().subscribe((response: ApiResponseModel<Array<PlayerServerModel>>) => {

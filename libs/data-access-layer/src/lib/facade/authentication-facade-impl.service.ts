...
import { UserAction, UserState } from '@wvs-play-win-workspace/data-access-store';

@Injectable ({
  providedIn: 'root'
})
export class AuthenticationFacadeImplService implements AuthenticationFacadeService {
  ...
  authenticateUser (authDataModel: AuthDataModel): Observable<any> {
    ...
    return this.authenticationService.authentication (param).pipe (tap (
      (x: ApiResponseModel<any>) => {
        if (x.status === 200) {
  ...
  validate (): Observable<any> {
    return this.authenticationService.validateToken ().pipe (tap (
      (x: ApiResponseModel<any>) => {
        if (x.status === 200) {
          if (x.data !== null && x.data !== '') {
  ...
  getProfile (): Observable<any> {
    return this.authenticationService.profile ().pipe (tap (
      (x: ApiResponseModel<any>) => {
        if (x.status === 200) {
          if (x.data !== null && x.data !== '') {
            const serverResponse: UserAuthenticationModel = x.data;

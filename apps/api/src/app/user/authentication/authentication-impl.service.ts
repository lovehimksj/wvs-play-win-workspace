...
import {
  AuthenticationDto,
  UserDto,
  UserTokenDto,Users, Wallet, UserTokens
...
export class AuthenticationImplService implements AuthenticationService {
    ...
    try {
      const userIsExist = await this.usersRepository.findOne ({ email: authenticationDto.email });
        ...
        if (isPasswordMatch) {
          ...
          const oldSession = await this.tokenRepository.findOne ({ userId: userIsExist.userId });
          if (oldSession && oldSession.isActive === 1) {
            await this.tokenRepository.delete ({ userId: userIsExist.userId });
          }
          const token = await this.jwtService.signAsync ({ userName: userIsExist.email, userId: userIsExist.userId });
          ...
          userToken.expirationDate = new Date ((new Date ()).setMilliseconds (60 * 60 * 1000));
          userToken.token = token;
          const userTokenDto: UserTokenDto = new UserTokenDto (userToken.expirationDate, userToken.createDate, userToken.token, 60 * 60 * 1000);
  ...
  async saveUser (userDto: UserDto, filename: string): Promise<Users> {
    ...
    userEntity.userId = 'SW_' + (Date.now ().toString (36) + Math.random ().toString (36).substr (2, 5)).toUpperCase ();
  ...
  async updateProfile (user: UserDto, userId: string): Promise<UpdateResult> {
    ...
    if (oldUser) {
      ...
      const updatedUser = await this.usersRepository.update ({ userId: oldUser.userId }, oldUser);

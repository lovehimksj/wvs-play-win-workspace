...
export class GameMasterImplService implements GameMasterService {
  ...
  async getGameById(gameId: number): Promise<GameMasterDto> {
    return await this.userRepository
      .findOne(gameId)
      .then((game) => this.gameHelperService.buildGameMaster(game));
  }
  ...
  async getAllGame(): Promise<Array<GameMasterDto>> {
    ...
    return await this.userRepository
      .find({ isActive: 1 })
      .then((game: Array<GameMaster>) => {
        game.forEach((value1: GameMaster) => {
          const gameMaster: GameMasterDto =
            this.gameHelperService.buildGameMaster(value1);
  ...
  async deleteGame(gameId: number): Promise<GameMasterDto> {
    return await this.userRepository
      .softDelete({ gameId: gameId })
      .then((game) => this.gameHelperService.buildGameMaster(game.raw));
  }

  async updateGame(
    updateGameDto: GameMasterDto,
    gameId: number
  ): Promise<GameMasterDto> {
    const gameMaster = new GameMaster();
    ...
    return await this.userRepository
      .update({ gameId: gameId }, gameMaster)
      .then((game) => this.gameHelperService.buildGameMaster(game.raw));
  }

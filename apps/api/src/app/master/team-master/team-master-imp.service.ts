...
export class TeamMasterImpService implements TeamMasterService {
  constructor(
    @InjectRepository(TeamMaster)
    private readonly teamMasterRepository: Repository<TeamMaster>,
    @InjectRepository(TeamTournamentDetail)
    private readonly teamTournamentDetailRepository: Repository<TeamTournamentDetail>
  ) {}
  ...
  async updateTeam(
    ...
    const teamMaster: TeamMaster = await this.teamMasterRepository.findOne(
      teamId
    );
    if (teamMaster) {
  ...
  async getTeamById(teamId: number): Promise<TeamMasterDto> {
    return await this.teamMasterRepository
      .findOne(teamId)
      .then((teamMaster: TeamMaster) => {
        if (teamMaster) {
  ...
  async getAllTeam(): Promise<Array<TeamMasterDto>> {
    return await this.teamMasterRepository
      .find()
      .then((teamMasters: Array<TeamMaster>) => {
  ...
  async getAllTeamsByTournament(
    tournamentId: number
  ): Promise<Array<TeamMasterDto>> {
    const teamIds: Array<TeamTournamentDetail> =
      await this.teamTournamentDetailRepository.find({
        tournamentId: tournamentId,
      });
    const teams: Array<TeamMaster> = await this.teamMasterRepository.findByIds(
      teamIds
    );
  ...
  async tournamentTeamMapping(
    teamsId: number[],
    tournamentId: number
  ): Promise<any> {
    try {
      if (teamsId.length > 0) {
        await this.teamTournamentDetailRepository.delete({
          tournamentId: tournamentId,
        });
        const teamTournamentDetailList: Array<TeamTournamentDetail> = [];
        teamsId.forEach((value) => {
          const teamTournamentDetail: TeamTournamentDetail =
            new TeamTournamentDetail();
        ...
        return await this.teamTournamentDetailRepository.save(
          teamTournamentDetailList
        );
      }
    } catch (e) {
      throw new Error(e);

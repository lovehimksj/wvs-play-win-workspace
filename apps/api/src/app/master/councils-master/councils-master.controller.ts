...
export class CouncilsMasterController {
  ...
  async findAll(@Res() res): Promise<void> {
    try {
      return await this.councilsMasterService
        .getAllCouncils()
        .then((value: Array<CouncilMasterDto>) => {
          const successResponseModel: SuccessResponseModel<
            Array<CouncilMasterDto>
          > = new SuccessResponseModel<Array<CouncilMasterDto>>(
            value,
            HttpStatus.OK,
  ...
  getCouncil(
    ...
    try {
      return this.councilsMasterService
        .findCouncilById(councilId)
        .then((value) => {
          const successResponseModel: SuccessResponseModel<CouncilMasterDto> =
            new SuccessResponseModel<CouncilMasterDto>();
  ...
  async saveCouncil(
    @Body() councilMasterDto: CouncilMasterDto,
    @Res() res: Response
  ): Promise<void> {
  ...
  async updateCouncil(
    @Query('id') councilId: number,
    @Body() councilDto: CouncilMasterDto,
    @Res() res: Response
  ): Promise<void> {
    return this.councilsMasterService
      .updateCouncil(councilId, councilDto)
      .then((value) => {
        const successResponseModel: SuccessResponseModel<CouncilMasterDto> =
          new SuccessResponseModel<CouncilMasterDto>();
        if (Object.entries(value).length === 0) {
  ...
  async findCouncilByGame(
    @Param('gameId') gameId: number,
    @Res() res
  ): Promise<void> {
    try {
      return await this.councilsMasterService
        .getAllCouncilsByGameId(gameId)
        .then((value: Array<CouncilMasterDto>) => {
          const successResponseModel: SuccessResponseModel<
            Array<CouncilMasterDto>
          > = new SuccessResponseModel<Array<CouncilMasterDto>>(

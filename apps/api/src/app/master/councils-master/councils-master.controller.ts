...
import { CouncilMasterDto, SuccessResponseModel } from '@wvs-play-win-workspace/api-interfaces';
  ...
  getCouncil(@Param('councilId') councilId: number, @Res () res: Response): Promise<void> {
    try {
      return this.councilsMasterService.findCouncilById(councilId).then (value => {
        const successResponseModel: SuccessResponseModel<CouncilMasterDto> = new SuccessResponseModel<CouncilMasterDto> ();

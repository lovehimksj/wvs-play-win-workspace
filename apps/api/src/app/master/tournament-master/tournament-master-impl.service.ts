...
export class TournamentMasterImplService implements TournamentMasterService {
  constructor(
    @InjectRepository(TournamentMaster)
    private readonly tournamentRepository: Repository<TournamentMaster>,
    @InjectRepository(CouncilTournamentDetail)
    private readonly councilTournamentDetailRepository: Repository<CouncilTournamentDetail>
  ) {}

  async getAllTournament(): Promise<Array<TournamentMasterDto>> {
    return this.tournamentRepository
      .find({ isActive: 1 })
      .then((value: Array<TournamentMaster>) => {
        const tournamentList: Array<TournamentMasterDto> = [];
        if (value && value.length > 0) {
          value.forEach((value1: TournamentMaster) => {
            if (value1) {
              const tournamentMaster: TournamentMasterDto =
                new TournamentMasterDto();
  ...
  async saveTournament(
    tournamentMasterDto: TournamentMasterDto,
    councilId: number
  ): Promise<TournamentMasterDto> {
    ...
    const saved = await this.tournamentRepository.save(tournamentMaster, {
      transaction: true,
    });
    Logger.log(JSON.stringify(saved), 'saved');
    if (saved) {
      const councilTournamentDetail: CouncilTournamentDetail =
        new CouncilTournamentDetail();
      ...
      const result = await this.councilTournamentDetailRepository.save(
        councilTournamentDetail,
        { transaction: true }
      );
      if (result) {
  ...
  async updateTournament(
    ...
    const tournamentMaster: TournamentMaster =
      await this.tournamentRepository.findOne({ tournamentId: tournamentId });
    Logger.log(JSON.stringify(tournamentMaster), 'tournamentMaster');
    if (tournamentMaster) {
      tournamentMaster.tournamentDese =
        tournamentMasterDto.tournamentDescription;
      tournamentMaster.tournamentName = tournamentMasterDto.tournamentName;
      tournamentMaster.year = tournamentMasterDto.year;
      tournamentMaster.updateDate = new Date();
      return await this.tournamentRepository.update(
  ...
  async getTournamentById(tournamentId: number): Promise<TournamentMasterDto> {
    ...
    Logger.log(JSON.stringify(tournamentMaster), 'tournamentMaster');
    if (tournamentMaster) {
      const tournamentMasterDto: TournamentMasterDto =
        new TournamentMasterDto();
      tournamentMasterDto.tournamentDescription =
        tournamentMaster.tournamentDese;
  ...
  async findTournamentByCouncilId(
    councilId: number
  ): Promise<Array<TournamentMasterDto>> {
    const tournamentIds = await this.councilTournamentDetailRepository.find({
      councilId: councilId,
    });
    if (tournamentIds && tournamentIds.length > 0) {
      const tournamentMasterList: Array<TournamentMaster> =
        await this.tournamentRepository.findByIds(tournamentIds);

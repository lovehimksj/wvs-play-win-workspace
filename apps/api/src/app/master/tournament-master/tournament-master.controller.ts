...
export class TournamentMasterController {
  ...
  async findAll(@Res() res): Promise<void> {
    try {
      return await this.tournamentMasterService
        .getAllTournament()
        .then((value: Array<TournamentMasterDto>) => {
          if (value.length > 0) {
            const successResponseModel: SuccessResponseModel<
              Array<TournamentMasterDto>
            > = new SuccessResponseModel<Array<TournamentMasterDto>>(
              value,
              HttpStatus.OK,
    ...
    try {
      return await this.tournamentMasterService
        ...
        .then((value: TournamentMasterDto) => {
          if (value) {
            const successResponseModel: SuccessResponseModel<TournamentMasterDto> =
              new SuccessResponseModel<TournamentMasterDto>(
                value,
                HttpStatus.OK,
                'Game is fetched Successfully'
              );
  ...
  async createTournament(
    @Query('councilId') councilId: number,
    @Body() tournamentMasterDto: TournamentMasterDto,
    @Res() res: Response
  ): Promise<void> {
    try {
      return await this.tournamentMasterService
        .saveTournament(tournamentMasterDto, councilId)
        .then((value: TournamentMasterDto) => {
          const successResponseModel: SuccessResponseModel<TournamentMasterDto> =
            new SuccessResponseModel<TournamentMasterDto>(
              value,
  ...
  async updateTournament(
    ...
    try {
      return await this.tournamentMasterService
        .updateTournament(tournamentMasterDto, tournamentId)
        .then((value: TournamentMasterDto) => {
  ...
  async getTournamentByCouncil(
    @Param('councilId') councilId: number,
    @Body() tournamentMasterDto: TournamentMasterDto,
    @Res() res: Response
  ): Promise<void> {
    try {
      return await this.tournamentMasterService
        .findTournamentByCouncilId(councilId)
        .then((value: Array<TournamentMasterDto>) => {
          Logger.log(JSON.stringify(value), 'value');
          const successResponseModel: SuccessResponseModel<
            Array<TournamentMasterDto>
